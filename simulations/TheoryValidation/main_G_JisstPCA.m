% theory validation for single-factor case; diagonal D, G-JisstPCA
% Software dependency: tensor toolbox (https://www.tensortoolbox.org)
addpath('JisstPCA/functions')
addpath('JisstPCA/simulation/functions')
filename = "JisstPCA/simulations/TheoryValidation/G-JisstPCA_res.csv";
...
for l = 1 : num_p
    ...
    for j = 1 : num_SNR
        ...
        Dx = SNR_scale * (sqrt(p) + sqrt(N)) * diag([1.5 1 0.8]);
        Dy = 1.2 * SNR_scale * (sqrt(p) + sqrt(N)) * diag([1 0.8]);
        X = squeeze(ttt(tensor(V*Dx*V'), tensor(u)));
        Y = squeeze(ttt(tensor(W*Dy*W'), tensor(u)));
        for k = 1 : nrep
            ii = ii + 1;
            rng(k)
            ...
            [u0, ~, ~] = svds(M, 1);
            [hat_u, hat_V, hat_W, hat_Dx, hat_Dy, loss] = dJisst_single_iter(X_obs, Y_obs, u0, rx, ry, lam, max_iter, true);
            for kk = 0 : max_iter
                if kk == 0
