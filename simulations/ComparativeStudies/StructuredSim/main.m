...
%% comparative study, structured data
% Software dependency: tensor toolbox (https://www.tensortoolbox.org)
addpath('JisstPCA/functions')
filename = "/Applications/Files/research/JisstPCA/JisstPCA_new/structure/res.csv";
...
for diagonal = [false true]
    for SNR_k = 1 : 6
        ...
        if diagonal 
            Dx{1} = dx(1) * diag([2 1.5 1.2]); Dx{2} = dx(2) * diag([2 1.6]);
            Dy{1} = dy(1) * diag([2 1.5 1.2]); Dy{2} = dy(2) * diag([2 1.6]);
        else
            for k = 1 : K
