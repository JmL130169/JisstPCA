%% comparative study, unstructured data, diagonal d setting
% Software dependency: tensor toolbox (https://www.tensortoolbox.org)
addpath('JisstPCA/functions')
filename = "/Applications/Files/research/JisstPCA/JisstPCA_new/unstructure/diagonal_d_res1.csv";
    ...
    for SNR_k = 1 : length(SNR_list)
        for orth_k = 1 : 2
            ...
            if orthogonal_list(orth_k)
                Dx{1} = dx0 * diag([2 1.5 1]); Dx{2} = dx0 * diag([1 0.8]);
                Dy{1} = dy0 * diag([2 1.5 1.1]); Dy{2} = dy0 * diag([1 0.8]);
            else
                Dx{1} = dx0 * diag([2 1.5 1.2]); Dx{2} = dx0 * diag([1 0.8]);
                Dy{1} = dy0 * diag([2 1.5 1.2]); Dy{2} = dy0 * diag([1 0.8]);
            end
